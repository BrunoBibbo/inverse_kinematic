...
# Resolución de la cinemática inversa mediante CCD (Cyclic Coordinate Descent).
...
th=[ 0., 0., 0.]
a =[ 5., 5., 5.]
lmin=[(-3*pi)/4, 0.,(-3*pi)/4] # límite mínimo de cada articulación
lmax=[ (3*pi)/4, 10.,(3*pi)/4] # límite máximo de cada articulación
...
while (dist > EPSILON and abs(prev-dist) > EPSILON/100.):
  ...
  for i in range(len(th)):
	...
	th[actual] = lmax[actual]
      # convertir el valor del ángulo al intervalo [-pi,pi]. Hay que aseguar que el angulo está en el rango [-pi, pi].
      # Si es positivo, le restamos vueltas completas (pi) hasta que esté dentro del rango y si es negativo sumarle vueltas.
      ...
      if a[actual] < lmin[actual]:
	a[actual] = lmin[actual]
      if th[actual] > lmax[actual]:
	a[actual] = lmax[actual]
      # limitar con el valor mínimo que puede tomar la articulación
      # limitar con el valor máximo que puede tomar la articulación
    O[i+1] = cin_dir(th,a)
